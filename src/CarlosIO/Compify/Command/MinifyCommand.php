...
class MinifyCommand extends Command
{
    protected function configure()
    {
        $this
            ...
            ->setHelp(<<<EOT
The <info>minify</info> command iterates through all
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // $this->minifyPackages($composer, $output, $verbose);
    }

    private function minifyPackages(Composer $composer, OutputInterface $output, $verbose)
    {
        $output->writeln('<info>Scanning packages</info>');
        $targets = array();
        foreach ($composer->getPackage()->getRequires() as $link) {
            $targets[] = $link->getTarget();
        }

        foreach ($composer->getPackage()->getDevRequires() as $link) {
            $targets[] = $link->getTarget();
        }
    ...
    private function selectPackages(Composer $composer, OutputInterface $output, $verbose, $requireAll = false)
    {
        $targets = array();
        $selected = array();
